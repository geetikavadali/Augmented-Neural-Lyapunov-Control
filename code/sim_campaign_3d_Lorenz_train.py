...
This script aims at designing an ANLC for a controlled Lorenz system.
...
def f_value(x, u, parameters):
    ...
    x_dot = [-sigma*(x[:, 0] - x[:, 1]) + u[:, 0],
             r*x[:, 0] - x[:, 1] - x[:, 0]*x[:, 2] + u[:, 1],
             x[:, 0] * x[:, 1] - b*x[:, 2] + u[:, 2]]
...
while out_iters < max_loop_number and not found_lyap_f: 
    ...
    while i_epoch < max_iters and not found_lyap_f:
        ...
        f0 = f_value(x_0, u0, parameters)  # evaluating system in f(0,0)
...
if execute_postpro:
    ...
    exec(open("postprocessing/postpro_3d_Lorenz.py").read())
    ...
    if(parameters['test_closed_loop_dynamics']):
        ...
        desired_x1 =     [0.0,                   0.0,          0]
        desired_x2 =     [0.0,                   0.0,          0]
        desired_x3 =     [0.0,                   0.0,          0]
        ...
        if parameters['trajectory_closed_loop'] and found_lyap_f:
            ...
            cl.plot_traj(sol_x, sol_V, final_dir_traj, samples_number, parameters)
