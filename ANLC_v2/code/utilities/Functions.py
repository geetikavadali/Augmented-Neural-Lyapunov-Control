...
def CheckLyapunov(vars_, f_out_sym, V, ball_lb, ball_ub, config, epsilon):    
    ...
    condition = dreal.logical_and(dreal.logical_imply(ball_in_bound, V >= 0),
                           dreal.logical_imply(ball_in_bound, lie_derivative_of_V <= epsilon))  # CAVEAT: origina version: lie_derivative_of_V <= epsilon 
...
def AddLieViolationsOrder6(x, 
    ...
    # Add violation of the Lie derivative negativity for a 6-dimensional system
    ...
    x5 = np.linspace(-gamma_overbar, gamma_overbar, grid_points)
    x6 = np.linspace(-gamma_overbar, gamma_overbar, grid_points)
...
def AddLieViolationsOrder10_v1(x, 
                               gamma_overbar, 
                               grid_points,
                               zeta_D,
                               verbose_info, 
    ...
    # Add violation of the Lie derivative negativity for a 10-dimensional system.
    # Updated version where points are shuffled
    ...
    X5 = np.linspace(- gamma_overbar, gamma_overbar, grid_points)
    X6 = np.linspace(- gamma_overbar, gamma_overbar, grid_points)
    X7 = np.linspace(- gamma_overbar, gamma_overbar, grid_points)
    X8 = np.linspace(- gamma_overbar, gamma_overbar, grid_points)
    X9 = np.linspace(- gamma_overbar, gamma_overbar, grid_points)
    X10 = np.linspace(- gamma_overbar, gamma_overbar, grid_points)
    x1, x2, x3, x4, x5, x6, x7, x8, x9, x10 = np.meshgrid(X1, X2, X3, X4, X5, X6, X7, X8, X9, X10)
    ...
    where_V_neg = all_violations# all_violations[0:zeta_D, :]
    x_V_neg = torch.zeros(where_V_neg.size()[0], x.shape[1])
    ...
    for iFals in range(where_V_neg.size(0)):
        ...
        c3 = where_V_neg[iFals][2]
        c4 = where_V_neg[iFals][3]
        c5 = where_V_neg[iFals][4]
        c6 = where_V_neg[iFals][5]
        c7 = where_V_neg[iFals][6]
        c8 = where_V_neg[iFals][7]
        c9 = where_V_neg[iFals][8]
        c10 = where_V_neg[iFals][9]
        x_V_neg[iFals] = torch.tensor([x1[c1, c2, c3, c4, c5, c6, c7, c8, c9, c10],
                                       x2[c1, c2, c3, c4, c5, c6, c7, c8, c9, c10],
                                       x3[c1, c2, c3, c4, c5, c6, c7, c8, c9, c10],
                                       x4[c1, c2, c3, c4, c5, c6, c7, c8, c9, c10],
                                       x5[c1, c2, c3, c4, c5, c6, c7, c8, c9, c10],
                                       x6[c1, c2, c3, c4, c5, c6, c7, c8, c9, c10],
                                       x7[c1, c2, c3, c4, c5, c6, c7, c8, c9, c10],
                                       x8[c1, c2, c3, c4, c5, c6, c7, c8, c9, c10],
                                       x9[c1, c2, c3, c4, c5, c6, c7, c8, c9, c10],
                                       x10[c1, c2, c3, c4, c5, c6, c7, c8, c9, c10]])    
    ...
    where_Vdot_pos = all_violations #all_violations[0:zeta_D, :]
    x_Vdot_pos = torch.zeros(where_Vdot_pos.size()[0], x.shape[1])
    ...
    for iFals in range(where_Vdot_pos.size(0)):
        ...
        c3 = where_Vdot_pos[iFals][2]
        c4 = where_Vdot_pos[iFals][3]
        c5 = where_Vdot_pos[iFals][4]
        c6 = where_Vdot_pos[iFals][5]
        c7 = where_Vdot_pos[iFals][6]
        c8 = where_Vdot_pos[iFals][7]
        c9 = where_Vdot_pos[iFals][8]
        c10 = where_Vdot_pos[iFals][9]
        x_Vdot_pos[iFals] = torch.tensor([x1[c1, c2, c3, c4, c5, c6, c7, c8, c9, c10],
                                          x2[c1, c2, c3, c4, c5, c6, c7, c8, c9, c10],
                                          x3[c1, c2, c3, c4, c5, c6, c7, c8, c9, c10],
                                          x4[c1, c2, c3, c4, c5, c6, c7, c8, c9, c10],
                                          x5[c1, c2, c3, c4, c5, c6, c7, c8, c9, c10],
